  ...
  ([repos-url page]
    (let
      ...
      {:next_page_url (get-in response [:links :next :href])
       :content (map #(templ/uritemplate (get % :pulls_url) {"number" "comments"})
                     (:body response))})))
...
(defn fetch-pr-comments
  ...
  ([pr-comments-url, page]
    (let
      ...
      {:next_page_url (get-in response [:links :next :href])
       :content (map #(:body %) (filter #(= (get-in % [:user :login]) current-user)
                                         (:body response)))})))

(defn fetch-all
  "api-call should be a partially bound fn"
  [api-call]
  (let [exec-fetch (fn exec-fetch [api-call]
                      (let [response (api-call)])
                        (if (:next_page_url response)
                          (concat (:content response) (exec-fetch yuck))))]))

(defn -main
  []
  (let [urls (:content (fetch-pr-comment-urls-for-repos))]
